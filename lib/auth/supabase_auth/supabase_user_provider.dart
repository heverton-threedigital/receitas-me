...
class ReceitasMeSupabaseUser extends BaseAuthUser {
  ReceitasMeSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> receitasMeSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ReceitasMeSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
